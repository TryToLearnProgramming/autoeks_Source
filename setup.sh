...
sed -i "s|image: z|image: ${repoc}|g" /autoeks/deployment.yaml
sed -i "s|REPO=|REPO=${repo}|g" /autoeks/bitbucket-pipelines.yml
sed -i "s|CLUSTER_N=|CLUSTER_N=${cluster_n}|g" /autoeks/bitbucket-pipelines.yml
sed -i "s|AWS_REGION=|AWS_REGION=${reg}|g" /autoeks/bitbucket-pipelines.yml
sed -i "s|CLUSTER_N:|CLUSTER_N: ${cluster_n}|g" /autoeks/.github/workflows/main.yml
sed -i "s|awsregion:|awsregion: ${reg}|g" /autoeks/.github/workflows/main.yml
eksctl create cluster -n $cluster_n -r $reg --kubeconfig=~/.kube/config
